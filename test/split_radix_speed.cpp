...
{
    int nsec;
    double ct = bench(N, name, runs, metaFFT_transformOut<Func, -1, Allocator<float_type>>, &metaFFT_clean<Allocator<float_type>>, nsec);
    printf("N = 2^%2.0f = %5i: %5i nsec, speed %s = %5.2f GFLOPS  FFTW/metaFFT = %4.1f\n", log2(N), N, nsec, name, ct, fftw_flops / ct);
}
...
/*
...

    {
        ...
        compareOut<fft<N, std::complex<float_type>, butterfly_policy>>(N, "split  ctran+unrolled ", runs, fftw_flops);
    }
